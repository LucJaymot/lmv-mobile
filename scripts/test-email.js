 ...
 * Version JavaScript (peut être exécuté directement avec Node.js)
 * 
 * Usage:
 *   node scripts/test-email.js votre-email@example.com
 */

// Note: Ce script nécessite que vous ayez configuré votre client Supabase
// Assurez-vous que lib/supabase.ts exporte bien le client Supabase
...
async function testEmail(to) {
  ...
  // Note: Vous devrez peut-être adapter le chemin selon votre structure
...
if (!email) {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  node scripts/test-email.js votre-email@example.com');
  console.error('');
  process.exit(1);
}
